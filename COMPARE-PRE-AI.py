...
df_pre = pd.read_csv('frequencies_pre.csv')
df_ai = pd.read_csv('frequencies_ai.csv')

# turn into sets for easier comparison
set_pre = set(df_pre['Word'])
set_ai = set(df_ai['Word'])

# comparison groups 
new_words = set_ai - set_pre     
lost_words = set_pre - set_ai  
common_words = set_pre & set_ai     
...
print("\nTOP 15 LOST WORDS")
df_lost = df_pre[df_pre['Word'].isin(lost_words)].copy()
...
df_pre_common = df_pre[df_pre['Word'].isin(common_words)][['Word', 'PMW']].rename(columns={'PMW': 'PMW_PRE'})
...
df_comp = pd.merge(df_pre_common, df_ai_common, on='Word', suffixes=('_PRE', '_AI'))

# calculating the difference and rounding later
df_comp['Difference_PMW'] = (df_comp['PMW_AI'] - df_comp['PMW_PRE']).round(1)
...
for idx, row in df_increase.head(10).iterrows():
    print(f"• {row['Word']}: +{row['Difference_PMW']} PMW (De {row['PMW_PRE']} a {row['PMW_AI']})")
...
df_increase.to_csv('comparison_ai.csv', index=False, encoding='utf-8-sig')
print("\nGenerated 'comparison_ai.csv' sorted by PMW increment.")
